...
var facultyMember = {
    ...
    name: "Bennett, Brian Todd (Full-time)",
    fall: "2019",
    course: "CSCI5960 Thesis In CSCI",
    qualification:"Academic",
};
objectArray.push(facultyMember);
var facultyMember = {
...
function addRow(tbl, position, currentObject){
    ...
    var temp = '<a href="#" class="cssLinks">' + currentObject.name + "</a>"
    addCell(tr, temp);

    temp = "Fall " + currentObject.fall;
    addCell(tr, temp);

    temp = currentObject.course;
    addCell(tr, temp);

    temp = currentObject.qualification;
    addCell(tr, temp);
...
function addHeader(thead){
    var strings = ["Faculty Name", "Semester", "Course", "Credential"];
...
function loadTable(selectedDepartment, selectedStartYear){
    ...
    for(var i = 0; i < objectArray.length; i++)
    {
        ...
        if (currentObject.fall === selectedStartYear && currentObject.department === departments[selectedDepartment])
        {
            added = addRow(tbl, added, currentObject);
        }
    }
}
...
function main(){
    ...
    loadTable(selectedDepartment, selectedStartYear);
}
