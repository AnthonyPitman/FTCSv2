...
function addRow(tbl, position, currentObject){
    ...
    var temp = currentObject.department;
    addCell(tr, temp);

    temp = currentObject.name;
    addCell(tr, temp);
...
function addHeader(thead){
    var strings = ["Department", "Faculty Name", "Courses Taught", "Academic Degrees", "Professional Qualificications"];
...
function loadTable(selectedStartYear){
    ...
    for(var i = 0; i < objectArray.length; i++)
    {
        ...
        if (currentObject.year === selectedStartYear)
        {
...
function main(){
    var temp = document.getElementById("StartYearSelectionID");
    var selectedStartYear = temp.value;

    loadTable(selectedStartYear);
}